...
window.addEventListener("load", function () {
    console.log(localStorage.getItem("randomPoke"))
    const randomPokemonJsonObj = localStorage.getItem("randomPoke");

    displayRandomPokemon(randomPokemonJsonObj);


    async function displayRandomPokemon(name) {
        let randomPokemonJsonObj;
        if (!name) {
            let randomPokemonObj = await fetch("https://trex-sandwich.com/pokesignment/pokemon?random=random");
            randomPokemonJsonObj = await randomPokemonObj.json();
            localStorage.setItem("randomPoke", randomPokemonJsonObj.name);
        } else {
            let clickedPokeObj = await fetch(`https://trex-sandwich.com/pokesignment/pokemon?pokemon=${name}`);
            randomPokemonJsonObj = await clickedPokeObj.json();
    ...
    window.setInterval(CheckIdleTime, 60 * 1000 * 5);

    function CheckIdleTime() {
        let localTime = localStorage.getItem("lastInteraction");
        //4 hours in mille seconds
        if (Date.now() - parseInt(localTime) >= 60 * 1000 * 60 * 4) {
            displayRandomPokemon(null);
        }
    }
    ...
    if (sessionStorage.getItem("onePoke")) {
        clickPokemon(sessionStorage.getItem("onePoke"));
    } else {
        displayDetails();
    }
    let allNamesArray = [];
    ...
    async function clickPokemon(name) {
        ...
        sessionStorage.setItem("onePoke", name);
